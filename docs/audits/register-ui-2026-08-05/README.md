...
- 현재 이메일/비밀번호 기반 flow에는 비밀번호 찾기/재설정 진입점과 안내 화면이 있으며, 실사용 전 백엔드 API 연결 form으로 확장한다.
...
- ToDoLab은 현재 소셜/패스키 없이 이메일 기반으로 시작하므로, 계정 생성 form은 짧게 유지하고 복구 흐름은 백엔드 API 준비 후 이메일 입력 form으로 연결한다.
