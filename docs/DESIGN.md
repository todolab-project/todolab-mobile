...
> 신뢰감 있는 파란색과 조밀하고 정돈된 목록으로 오늘의 실행을 빠르게 파악하고 시작하게 한다.

UI/UX 개편 방향:

- 화면을 큰 카드가 반복되는 대시보드보다 빠르게 훑을 수 있는 목록으로 구성한다.
...
- Web 콘텐츠 최대 너비는 기본 720px이며 중앙 정렬한다. 목록이 지나치게 넓어지면 화면별로 더 좁은 readable width를 사용한다.
...
| ----------- | ------------ | --------------------------------------------------------------- |
| compact     | 320–359px/pt | 좌우 16px, 설명과 metadata 우선 축약, 수평 버튼 묶음 금지       |
...
- Task의 기본 표현은 큰 독립 카드보다 divider로 구분한 compact list row다.
- 한 줄 Task의 목표 높이는 52–60px이며, 제목이 두 줄이거나 metadata가 있을 때만 자연스럽게 확장한다.
- 완료 control의 보이는 원 또는 glyph는 20–24px로 유지하고 독립된 hit area는 최소 44×44pt로 제공한다.
...
- Card가 필요한 D-Day 목표나 요약과 일반 Task row를 시각적으로 구분한다.
...
- Today: compact header 다음에 실행 Task를 두고 일정, 보조 진입점, 접힌 완료 순으로 표시한다.
- Calendar: compact header, 주/월 전환, 날짜 선택, 선택 날짜 Task 목록 순서를 유지한다.
- D-Day: compact header의 추가 행동, 목표 목록, 필요할 때 펼치는 연결 Task 순서를 사용한다.
- More: 큰 destination card 대신 52–60px navigation row와 divider를 사용한다.
- Inbox: category section과 compact Task row를 사용하고 Today/내일 이동은 swipe 또는 overflow action으로 제공한다.
- Completed: 날짜 선택과 조용한 요약을 compact하게 두고 완료 Task 목록을 우선한다.
...
- VoiceOver, TalkBack, keyboard 사용자를 위해 overflow menu에 “위로 이동”, “아래로 이동” 대체 행동을 유지한다.
