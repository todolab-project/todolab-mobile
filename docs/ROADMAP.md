...
다음 모바일 작업은 Phase 6의 완료 section 기본 접힘 처리다. 완료 목록은 기본 접힘 상태로 두고 완료 수와 펼치기 행동만 표시해 실행 흐름을 밀어내지 않게 한다. Phase 3의 주간/월간 날짜 셀 상태 점과 개수는 백엔드 `DAY`, `WEEK`, `MONTH` 범위 조회 계약이 확정된 뒤 연결한다.
