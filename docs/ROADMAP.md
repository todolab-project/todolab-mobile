...
- [ ] 320px Web, 375pt iPhone, 430dp Android에서 현재 화면의 first viewport 기준선 기록
...
다음 모바일 구현은 Phase 3의 Today와 Calendar 날짜 일관성 회귀 테스트다. 주간/월간 날짜 셀의 상태 점과 개수는 백엔드 `DAY`, `WEEK`, `MONTH` 범위 조회 계약이 확정된 뒤 연결한다.
