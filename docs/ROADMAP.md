...
다음 모바일 작업은 Phase 6의 `오늘의 흐름` 중복 제목과 5분할 요약 카드 정리다. 실행 Task 이후에 남아 있는 요약 정보가 목록을 다시 밀어내지 않도록 한 줄 진행 요약으로 축소하거나 제거한다. Phase 3의 주간/월간 날짜 셀 상태 점과 개수는 백엔드 `DAY`, `WEEK`, `MONTH` 범위 조회 계약이 확정된 뒤 연결한다.
