...
다음 모바일 작업은 Phase 6의 Task 작성·상세 form compact 개편이다. 제목·날짜·시간을 우선 배치하고 설명·카테고리·D-Day 같은 낮은 빈도 필드는 점진적으로 펼칠 수 있는 구조를 검토한다. 저장 행동은 키보드와 safe area에 가리지 않게 유지하고 삭제는 상세 하단의 분리된 위험 영역에 둔다. Completed는 완료 card 목록을 주간 요약보다 먼저 표시하고 다시 열기는 `⋯`에서만 제공한다. 재정렬은 백엔드에 [`API_TODAY_REORDER.md`](./API_TODAY_REORDER.md)의 단일 mutation 계약이 구현되기 전까지 기존 `UP`/`DOWN` API를 순차 호출한다. Phase 3의 주간/월간 날짜 셀 상태 점과 개수는 백엔드 `DAY`, `WEEK`, `MONTH` 범위 조회 계약이 확정된 뒤 연결한다.
